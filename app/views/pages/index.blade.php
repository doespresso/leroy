...
<div id="page-{{$page->alias}}" class="page-slide @if (!($page->gallery))vm@endif" data-hash="page:{{$page->alias}}" data-photos="{{!empty($page->gallery->id) ? $page->gallery->id:''}}" data-photos-loaded="0" style="background-color: {{$page->bgcolor}}; color:{{$page->color}}">
    <div id="{{$page->alias}}-pages" class="pages-container">
        <div class="pages-wrapper">
            <?php
            if ($page->gallery) {
            ...
            <div class="page-slide" data-type="photo" data-id="{{$photo->id}}" @if($photo->dark_content!=1) data-mutate="mutator-bg-dark"@endif style='background-image:url({{url("/")}}/uploads/gallery/big/{{$photo->img}})'>
